...
// ═══════════════════════════════════════════════════════════════
// تابع بررسی و مدیریت اشتراک
// ═══════════════════════════════════════════════════════════════
function checkSubscription() {
    // بررسی وجود تنظیمات اشتراک و وضعیت انقضا
    if (CAFE_CONFIG.subscription && CAFE_CONFIG.subscription.isExpired) {
        ...
        const subConfig = CAFE_CONFIG.subscription;

        // تنظیم محتوای پاپ‌آپ از فایل کانفیگ
        titleEl.textContent = subConfig.popupTitle || 'اشتراک منقضی شده';
        messageEl.textContent = subConfig.popupMessage || 'لطفا اشتراک خود را تمدید کنید.';
        buttonEl.textContent = subConfig.buttonText || 'تمدید';
        buttonEl.href = subConfig.buttonLink || '#';

        // نمایش پاپ‌آپ
        overlay.classList.add('visible');
        
        // جلوگیری از اسکرول صفحه در پس‌زمینه
        document.body.style.overflow = 'hidden';

        return true; // به معنی "منقضی شده"
    }
    return false; // به معنی "فعال"
}


// تابع برای اعمال استایل‌های ضروری (فونت و رنگ) در ابتدای کار
...
// 1. ابتدا استایل‌های ضروری را اعمال کن تا پاپ‌آپ هم ظاهر درستی داشته باشد
applyEssentialStyles();

// 2. سپس وضعیت اشتراک را بررسی کن
const isExpired = checkSubscription();

// 3. اگر اشتراک فعال بود، بقیه برنامه را اجرا کن
if (!isExpired) {
    initializeMenu();
    setupSocialButtons();
}
